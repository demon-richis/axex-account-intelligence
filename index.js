...
const app=express(); app.disable('x-powered-by'); app.set('trust proxy',1); app.use(cors()); app.use(express.json({limit:'100kb'})); app.use(rateLimit); app.get('/health',(req,res)=>res.json({status:'ok',service:'axex-intelligence',analyzedAt:new Date().toISOString()})); app.use(auth);
